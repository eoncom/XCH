...
echo ""
echo "  1) Nginx integre  — XCH gere tout (reverse proxy inclus)"
echo "                       Ports HTTP/HTTPS personnalisables"
echo ""
echo "  2) Nginx externe   — Vous avez deja un reverse proxy"
echo "                       (Nginx Proxy Manager, Traefik, Apache...)"
echo "                       XCH expose backend + frontend sur des ports"
echo ""
echo "  3) Developpement   — Tous les ports exposes pour debug"
echo "                       Backend, Frontend, PostgreSQL, Redis, MinIO"
echo ""
...
if [ "$MODE" = "3" ]; then
    ...
    # On utilise le docker-compose.yml de base (pas d'override dev)
    # mais on expose les ports via un override genere
    ...
    # Generer un override pour exposer uniquement backend + frontend
...
EXTEOF
    ...
    echo "    /         -> http://localhost:${CUSTOM_FRONTEND_PORT}"
    echo "    /api/     -> http://localhost:${CUSTOM_BACKEND_PORT}"
    echo "    /storage/ -> http://localhost:9000  (MinIO, si acces direct necessaire)"
    ...
    BACKEND_PORT=${CUSTOM_HTTP_PORT}
    FRONTEND_PORT=${CUSTOM_HTTP_PORT}
    NODE_ENV="production"
    USE_NGINX="integrated"
    ...
    if [ "$CUSTOM_HTTP_PORT" = "80" ]; then
        ...
        APP_URL="http://localhost:${CUSTOM_HTTP_PORT}"
    fi
...
# ===== EXPOSED PORTS (for external proxy mode) =====
BACKEND_PORT=${CUSTOM_BACKEND_PORT:-3002}
FRONTEND_PORT=${CUSTOM_FRONTEND_PORT:-3001}
...
if [ "$MODE" = "3" ]; then
    ...
    echo -e "    ${BLUE}http://${SERVER_IP}:${FRONTEND_PORT}/setup${NC}"
    echo ""
    echo "  Ports exposes pour votre reverse proxy :"
    echo "    Frontend : http://${SERVER_IP}:${FRONTEND_PORT}"
    echo "    Backend  : http://${SERVER_IP}:${BACKEND_PORT}"
    echo ""
    echo "  Configuration reverse proxy :"
    echo "    votre-domaine.com/         -> http://127.0.0.1:${FRONTEND_PORT}"
    echo "    votre-domaine.com/api/     -> http://127.0.0.1:${BACKEND_PORT}"
    echo "    votre-domaine.com/storage/ -> http://127.0.0.1:9000"
else
    if [ "${CUSTOM_HTTP_PORT:-80}" = "80" ]; then
        echo -e "    ${BLUE}http://${SERVER_IP}/setup${NC}"
    else
        echo -e "    ${BLUE}http://${SERVER_IP}:${CUSTOM_HTTP_PORT}/setup${NC}"
    fi
fi
echo ""
