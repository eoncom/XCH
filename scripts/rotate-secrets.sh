...
#
# Modes:
#   --phase a   (défaut) JWT_SECRET, JWT_REFRESH_SECRET, MINIO_*
...
#     7. docker compose up -d --no-deps --force-recreate minio backend
...
#   (cp .env.bak.XXX .env && docker compose up -d backend). Pas de rollback
...
# ── PHASE A ─────────────────────────────────────────────────────────────────
phase_a() {
    print_header "PHASE A — JWT + MinIO"
    ...
    echo -e "\n${YELLOW}Cibles : 64/64/20/40/40/40 chars.${NC}"
    ...
    local NEW_JWT NEW_REFRESH NEW_MINIO_AK NEW_MINIO_SK
    ...
    echo "  ✓ 4 secrets générés (longueurs : 64 / 64 / 20 / 40)"
    ...
    echo "  ✓ 6 set_env appliqués (backend + root)"
    ...
    echo -e "\n${BLUE}5. Restart rolling : minio + backend${NC}"
    if [ "$DRY_RUN" = false ]; then
        ...
        docker compose up -d --no-deps --force-recreate minio || {
            ...
            echo "  docker compose up -d minio backend"
            return 1
        }
        sleep 3
        echo "  → restart backend (lit les nouveaux JWT + MinIO)"
        docker compose up -d --no-deps --force-recreate backend
    else
        echo "  [dry-run] docker compose up -d --no-deps --force-recreate minio backend"
    fi
    ...
    if [ "$DRY_RUN" = false ]; then
        ...
        if healthcheck "$HEALTH_URL" 90; then
            ...
            echo "    cp ${BACKEND_ENV}.bak.${TS} $BACKEND_ENV"
            echo "    cp ${ROOT_ENV}.bak.${TS} $ROOT_ENV"
            echo "    docker compose up -d --force-recreate minio backend"
            return 1
        fi
    fi
